...
import {
  ...
  Plus, Minus, CreditCard, CheckCircle, Fingerprint, ShieldCheck,
...
// ── Inline Product Card ───────────────────────────────────────────────────────
// phase: idle → adding → ready → card-select → passkey → checkout → confirmed
function ChatProductCard({ product, onOrderConfirmed }) {
  ...
  const handleVerifyPasskey = async () => {
    ...
    try {
      ...
      const msg = err.name === 'NotAllowedError'
        ? 'The browser prompt was dismissed or no passkey is available.'
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-3 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between pt-1.5 border-t border-green-200">
          <span className="text-[10px] text-green-600 flex items-center gap-1">
            ...
            Passkey verified · {order.cardBrand} •••• {order.cardLast4}
          </span>
  ...
  const isActive = ['ready', 'card-select', 'passkey', 'checkout'].includes(phase)

  return (
    <div className={`bg-white border rounded-lg overflow-hidden shadow-amex-sm transition-all duration-200 flex flex-col ${
      isActive ? 'border-amex-blue shadow-amex-md' : 'border-surface-border hover:border-amex-blue hover:shadow-amex-md'
    }`}>
      {/* Image */}
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        ...
        {/* ── Phase: ready — Proceed to Checkout ── */}
        {phase === 'ready' && (
          <div className="pt-2 border-t border-amex-blue/20">
            <button onClick={() => setPhase('card-select')}
              className="w-full flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded bg-amex-blue hover:bg-amex-dark text-white transition-all">
              <CreditCard className="w-3.5 h-3.5" /> Proceed to Checkout
            </button>
          </div>
        )}

        {/* ── Phase: card-select — Select card + Continue ── */}
        {phase === 'card-select' && (
          <div className="flex flex-col gap-2 pt-2 border-t border-amex-blue/20">
            <p className="text-[10px] font-semibold text-ink-secondary uppercase tracking-wide">Select Payment Card</p>
            {availableCards.length > 0 ? (
              <select
                value={selectedCard?.id || ''}
                onChange={e => setSelectedCard(availableCards.find(c => c.id === e.target.value))}
                className="w-full text-[11px] border border-surface-border rounded px-2 py-1.5 bg-white text-ink-primary focus:outline-none focus:border-amex-blue"
              >
            ...
            <button
              onClick={() => { setPasskeyError(''); setPhase('passkey') }}
              disabled={!selectedCard}
              className="w-full flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded bg-amex-blue hover:bg-amex-dark text-white disabled:opacity-60 transition-all"
            >
              Continue with Selected Card <ChevronRight className="w-3.5 h-3.5" />
            </button>
          </div>
        )}

        {/* ── Phase: passkey — Verify identity ── */}
        {phase === 'passkey' && (
          <div className="flex flex-col gap-2 pt-2 border-t border-amex-blue/20">
            {/* Card summary */}
            <div className="flex items-center gap-2 bg-surface-light border border-surface-border rounded px-2 py-1.5">
              <CreditCard className="w-3.5 h-3.5 text-amex-blue shrink-0" />
              <span className="text-[11px] text-ink-primary font-medium flex-1">
                {selectedCard?.brand} •••• {selectedCard?.last4}
              </span>
              <span className="text-[11px] font-bold text-amex-blue">
            ...
            <div className="flex flex-col items-center gap-1.5 py-2">
              <div className="w-10 h-10 rounded-full bg-amex-light border border-amex-blue/30 flex items-center justify-center">
                <Fingerprint className="w-5 h-5 text-amex-blue" />
              </div>
              <p className="text-[11px] text-ink-secondary text-center leading-snug">
                Authenticate with your device passkey to complete the purchase.
              </p>
            </div>
            ...
            {passkeyError && (
              <div className="flex flex-col gap-1.5">
                <div className="flex items-start gap-1.5 bg-red-50 border border-red-200 rounded px-2 py-1.5">
                  ...
                  <p className="text-[10px] text-red-700">{passkeyError}</p>
                </div>
                <button onClick={placeOrder}
                  className="w-full flex items-center justify-center gap-1.5 text-[10px] font-semibold py-1.5 rounded border border-amber-200 text-amber-700 bg-amber-50 hover:bg-amber-100 transition-colors">
                  <span className="text-[9px] font-bold uppercase bg-amber-200 text-amber-800 px-1 rounded">Sandbox</span>
                  Continue in Demo Mode
                </button>
              </div>
            )}

            <button onClick={handleVerifyPasskey}
              className="w-full flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded bg-amex-blue hover:bg-amex-dark text-white transition-all">
              <ShieldCheck className="w-3.5 h-3.5" /> Verify Identity &amp; Continue
            </button>
          </div>
        )}

        {/* ── Phase: checkout — processing ── */}
        {phase === 'checkout' && (
          <div className="pt-2 border-t border-amex-blue/20">
            <div className="w-full flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded bg-amex-blue/70 text-white cursor-not-allowed">
